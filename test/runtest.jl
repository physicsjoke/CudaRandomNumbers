...
@testset "CudaRandomNumbers.jl" begin
    rng = CUDA.RNG()
    @testset "FloatType = $FloatType" for FloatType in [Float32]
            ...
            @testset "Parameter regimes" begin
                @testset "Uniform input array" begin
                    function test_mean_variance(A, λ, N)
                        @test mean(A) ≈ var(A) rtol=sqrt(100/N) atol=1.0f-7
                        @test mean(A) ≈ λ rtol=sqrt(100/N) atol=1.0f-7
                    end

                    function test_higher_cumulants(A, λ, N, n_cumulants)
                        for i in 3:n_cumulants
                            @test cumulant(A, i) ≈ λ rtol=sqrt(100/N) atol=1.0f-7
                        end
                    end
                    @testset "GPU: λ = $λ, N = $N" for λ in [1.0f0, 5.0f0, 30.0f0], N in [2^10, 2^16, 2^19, 2^22]
                    ...
                    @testset "CPU: λ = $λ, N = $N" for λ in [1.0f0, 5.0f0, 30.0f0], N in [2^10, 2^16, 2^19, 2^22]
                        #A = ones(FloatType, N)
                        #CudaRandomNumbers.poisson_rho_dep!(A, λ)
                        #A_copy = Array(A)
                        #test_mean_variance(A_copy, λ, N)
                        #test_higher_cumulants(A_copy, λ, N, 10)

                        B = ones(FloatType, N)
                        Random.rand!(Distributions.Poisson(λ),B)
                        test_mean_variance(B, λ, N)
                        test_higher_cumulants(B, λ, N, 10)
                    end
                end
            ...
            @testset "Parameter regimes" begin
                @testset "Uniform input array" begin
                    function test_mean_variance(A, k, N)
                        @test mean(A) ≈ k rtol=sqrt(100/N) atol=1.0f-7
                        @test var(A) ≈ k rtol=sqrt(100/N) atol=1.0f-7
                    end
